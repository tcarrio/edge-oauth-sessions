...
import {
	mapper as auth0Mapper,
	Auth0OIDCClient,
	Auth0OIDCOptionsSchema,
	EnvAuth0OIDCOptionsSchema,
} from "../auth0/auth0-oidc-client";
...
export class OpenIDConnectClientFactory {
	...
	static withFetchClient(): OpenIDConnectClientFactory {
		return new OpenIDConnectClientFactory(
			new FetchHttpClient(FetchHttpClientConfig.default()),
		);
	}
	...
	forEnv(env: unknown): OpenIDConnectClient {
		...
		switch (strategy) {
			case Strategy.Auth0:
				const { authorizationUri, ...auth0Options } = auth0Mapper(
					EnvAuth0OIDCOptionsSchema.parse(env),
				);

				return new Auth0OIDCClient(
					new AuthenticationClient({ ...auth0Options }),
					authorizationUri,
				);

			case Strategy.WorkOS:
				const { apiKey, ...workOSOptions } = workOSMapper(
					EnvWorkOSOIDCOptionsSchema.parse(env),
				);

				// TODO: Zod schema parity with interface
				return new WorkOSOAuthClient(
					new WorkOS(apiKey),
					workOSOptions as WorkOSOIDCOptions,
				);
			...
			default:
				const genericOptions = genericMapper(
					EnvGenericOIDCOptionsSchema.parse(env),
				);
	...
	forStrategy(
		...
		switch (strategy) {
			case Strategy.Auth0:
				const { authorizationUri, ...auth0Options } =
					Auth0OIDCOptionsSchema.parse(options);
			...
			case Strategy.WorkOS:
				...
				// TODO: Zod schema parity with interface
				return new WorkOSOAuthClient(
					new WorkOS(workOSOptions.apiKey),
					workOSOptions as WorkOSOIDCOptions,
				);
			...
			default:
				...
				return new GenericOIDCClient(
					genericOptions as GenericOIDCOptions,
					this.httpClient,
				);
		}
	}
