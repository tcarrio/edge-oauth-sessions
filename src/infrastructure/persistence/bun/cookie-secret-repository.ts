...
export class SqliteCookieSecretRepository implements CookieSecretRepository {
	...
	async delete(id: string): Promise<void> {
		const statement = this.db.prepare<never, [string]>(this.statementTemplates.delete);

		await statement.run(id);
	}

	async expire(time = Date.now()): Promise<void> {
		const statement = this.db.prepare<never, [number]>(this.statementTemplates.expire);
