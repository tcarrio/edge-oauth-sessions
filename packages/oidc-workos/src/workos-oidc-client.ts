...
export class WorkOSOIDCClient implements OpenIDConnectClient {
	constructor(
	...
	getAuthorizationUrl({
		...
		return this.workOS.userManagement.getAuthorizationUrl({
			...
			clientId:
				options.client_id ??
				this.options.authorization.client_id ??
				this.options.clientId,
			redirectUri:
				options.redirect_uri ?? this.options.authorization.redirect_uri,
	...
	private coerceScreenHint(
		screenHint?: ScreenHintType,
	): AuthorizationURLOptions["screenHint"] {
		return screenHint
			? screenHint === "SignUp"
				? "sign-up"
				: "sign-in"
...
export const mapper = mapperForMapping<
	WorkOSOIDCOptions,
	typeof EnvWorkOSOIDCOptionsSchema
>({
